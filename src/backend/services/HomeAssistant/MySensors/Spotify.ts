...
const typeboxAccounts = [t.Literal("Guilherme")];

interface SpotifyType {
	typeboxAccounts: typeof typeboxAccounts;
	accounts: readonly ["Guilherme"];
	entities: Record<SpotifyAccounts, string>;
	getSensor: (account: SpotifyAccounts) => MediaPlayer;
	...
	playSong: (account: SpotifyAccounts, uri: string) => Promise<void>;
	playAlbum: (account: SpotifyAccounts, uri: string) => Promise<void>;
	playArtist: (account: SpotifyAccounts, uri: string) => Promise<void>;
}

export const Spotify: SpotifyType = {
	typeboxAccounts: typeboxAccounts,
